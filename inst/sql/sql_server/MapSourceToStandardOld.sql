...
FROM (
	...
	FROM @old_vocabulary_database_schema.CONCEPT C
		JOIN @old_vocabulary_database_schema.CONCEPT_RELATIONSHIP CR
			ON C.CONCEPT_ID = CR.CONCEPT_ID_1
			AND CR.invalid_reason IS NULL
			AND lower(cr.relationship_id) = 'maps to'
		JOIN @old_vocabulary_database_schema.CONCEPT C1
	...
	FROM @old_vocabulary_database_schema.source_to_concept_map stcm
		LEFT OUTER JOIN @old_vocabulary_database_schema.CONCEPT c1
